...
import { Shield, Key, Bell, CreditCard, Check, Sparkles } from 'lucide-react';
        ...
        <main className="p-8 space-y-6 max-w-4xl mx-auto">
          ...
          <div className="rounded-2xl border border-slate-200/90 bg-white p-6 shadow-xs space-y-4">
            ...
            <div className="flex items-center justify-between text-xs text-slate-600">
              <span>Stripe Price ID: <code className="bg-slate-100 px-2 py-0.5 rounded text-blue-600 font-mono">price_1UAnwaKkUbVxosGgjAKYerlu</code></span>
              <button
                onClick={() => setIsPaidAdmin(!isPaidAdmin)}
                className="text-xs font-semibold text-blue-600 hover:underline"
              >
                Toggle Status ({isPaidAdmin ? 'Active' : 'Inactive'})
              </button>
            </div>
          </div>
