...
export default function DeveloperPortalPage() {
  ...
  const [tasksState, setTasksState] = useState([
    { id: 1, text: 'Implement Checkout UI & Responsive Grid', completed: true, priority: 'High', project: 'Nova App UI Redesign' },
    { id: 2, text: 'Fix Bug #415: JWT expiration race condition', completed: true, priority: 'High', project: 'Nova App UI Redesign' },
    { id: 3, text: 'Integrate Stripe SDK Sheet and Apple Pay Handlers', completed: false, priority: 'Medium', project: 'Nova App UI Redesign' },
    { id: 4, text: 'Write Unit Tests for Order Webhook Payload Validation', completed: false, priority: 'Medium', project: 'Enterprise Q4 Goals Tracker' },
  ]);
  ...
  const myProjects = projects.filter((p) =>
    (p.assigned_dev_ids || []).includes(currentUser.id)
  );
  const displayProjects = myProjects.length > 0 ? myProjects : projects.slice(0, 2);

  // Current developer's reports
  const myReports = workReports.filter((r) => r.developer_id === currentUser.id);
  const displayReports = myReports.length > 0 ? myReports : workReports.slice(0, 3);
  const onTimeCount = displayReports.filter((r) => r.is_on_time).length;
  const onTimePct = displayReports.length > 0 ? Math.round((onTimeCount / displayReports.length) * 100) : 98;
      ...
      <header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b border-slate-200 bg-white/95 px-6 backdrop-blur-md">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-slate-900">Developer Portal</span>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200/90 bg-white p-6 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Front-end Engineer · You have <strong>{tasksState.filter((t) => !t.completed).length} pending tasks</strong> scheduled for today.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="rounded-xl border border-slate-100 bg-slate-50 px-4 py-2 text-center">
              <p className="text-[10px] text-slate-400 font-bold uppercase">Clock</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="rounded-2xl border border-slate-200/90 bg-white p-6 shadow-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {displayProjects.map((p) => (
                  <div
                    key={p.id}
                    className="rounded-xl border border-slate-200 p-4 bg-slate-50/50 hover:bg-white hover:border-blue-300 transition"
                  >
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="rounded-2xl border border-slate-200/90 bg-white p-6 shadow-xs space-y-4">
              <h2 className="text-base font-bold text-slate-900">Developer Personal Statistics</h2>
              ...
              <div className="rounded-xl border border-emerald-100 bg-emerald-50/60 p-4 text-center">
                ...
                <span className="inline-flex items-center gap-1 rounded-full bg-emerald-200/60 px-2.5 py-0.5 text-[10px] font-bold text-emerald-900">
                  <CheckCircle2 className="h-3 w-3 text-emerald-700" /> High Compliance Streak
                </span>
              </div>
              ...
              <div className="rounded-xl border border-blue-100 bg-blue-50/60 p-4 text-center">
                ...
                <div className="my-2 text-3xl font-black text-blue-800">{currentUser.total_hours_logged || 164} hrs</div>
                <p className="text-[10px] text-blue-600">Tracked across all assigned deliverables</p>
              </div>
